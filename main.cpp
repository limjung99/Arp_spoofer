...
/* 
	Sender가 송신한 패킷을 relay하는 함수
*/
void relayPacket(pcap_t* handle){
	while(true){
		const u_char* packet;
		struct pcap_pkthdr* header;
		int res = pcap_next_ex(handle, &header, &packet); //큐에서 패킷을 찾음 
		struct libnet_ether_hdr *ethhdr = (struct libnet_ether_hdr*)packet;
		if(ntohs(ethhdr->type)!=0x0080) continue; //ip 프로토콜이 아니면 폐기
		struct libnet_ipv4_hdr *iphdr = (struct libnet_ipv4_hdr*)(packet+sizeof(ethhdr));
		u_int32_t ipsrc = iphdr->ip_src.s_addr;
		u_int32_t ipdst = iphdr->ip_dst.s_addr;
